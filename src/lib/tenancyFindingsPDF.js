...
// v0.4 — Variant C photo grid, emoji-stripped, ASCII arrow, footer URL, last-page cert
// ═══════════════════════════════════════════════════════════════════════════
...
//                                       Variant C 4-up letterbox grid
//   4. Certification footer (rendered on last page bottom margin)
//
// Public API:
...
//                       tenantMoveOut, precedingTurnover },
//     items: [
...
//   }
//
// EMOJI / FOOTER / CERT / PHOTO LAYOUT — see comparisonPDF.js for v0.4 conventions.
// ═══════════════════════════════════════════════════════════════════════════
...
export async function buildTenancyFindingsPDF(report, property, tenancy, photoStore) {
  ...
  for (const r of recordList) {
    ...
    doc.setFontSize(7); doc.setFont('helvetica', 'bold');
    // ASCII checkmark / open-circle equivalents
    doc.text((r.present ? '[x] ' : '[ ] ') + r.label, pillX + 2, y + 3.5);
    pillX += pillW + 3;
  }
  y += 9;
...
function renderFindingRow(doc, y, checkY, item, tierBgRgb, tierFgRgb) {
  ...
  // Room name header line — emoji-stripped (no roomIcon)
  doc.setTextColor(28, 25, 23);
  doc.setFontSize(9); doc.setFont('helvetica', 'bold');
  doc.text(item.roomName, MARGIN + 3, y + 5);
  ...
  for (const p of photos) {
    ...
    if (imgData) {
      try {
        ...
        // image failed; placeholder bg already drawn
      }
    } else {
      doc.setTextColor(160, 155, 150);
      doc.setFontSize(7); doc.setFont('helvetica', 'italic');
